...
class ExcelBot:
    def __init__(self, workbook_koi, worksheet_koi):
        ...
        self.cells_containing, self.workbooks, self.worksheets, self.worksheet_data, self.worksheet_data_of_interest = \
    ...
    def search_worksheets_of_interest(self, search_terms):
        try:
            ...
            for search_term in search_terms:
                ml.log_event('searching worksheets of interest for search term: {}'.format(search_term))
                self._record_cells_which_contain(search_term)
    ...
    def _record_cells_which_contain(self, search_term):
        ml.log_event('record cells that contain search_term: {}'.format(search_term), event_completed=False)
        try:
            self.cells_containing[search_term] = list()
            for worksheet_title, worksheet_data in self.worksheet_data.items():
                for cell_address, cell_value in worksheet_data.items():
                    if search_term in str(cell_value):
                        ml.log_event('search term: {} found at cell {} in value: {}'.format(
                            search_term, cell_address, cell_value))
                        self.cells_containing[search_term].append('CELL {}, SHEET {}, WORKBOOK {}'.format(
                            cell_address, worksheet_title, worksheet_data['parent_workbook']))
            ml.log_event('record cells that contain search_term: {}'.format(search_term), event_completed=True)
        except RuntimeError as r_err:
            ml.log_event(r_err)
